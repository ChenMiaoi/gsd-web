...
export const UI_COPY: Record<Locale, UiCopy> = {
  ...
  zh: {
    ...
    app: {
      ...
      healthRailCopy: '用于查看本地 GSD 工作区引导状态、身份连续性和监控时效的信息密集型控制台。',
    ...
    labels: {
      ...
      projectDetail: '项目详情',
      projectSwitch: '项目切换器',
      monitorFreshness: '监控时效',
      projectContinuity: '项目身份连续性',
      initialization: '初始化',
      ...
      risk: '风险',
      modelUsage: '模型使用',
      recentAgentUnits: '最近 Agent 执行单元',
      primaryModel: '主要模型',
      averageUnitDuration: '平均执行单元耗时',
      serviceState: '服务状态',
    },
    help: {
      ...
      inventoryProjection: '来自 /api/projects 的当前清单视图。',
      detailProjection: '来自 /api/projects/:id 的真实快照，并支持手动刷新。',
      monitor: '服务侧状态校准健康度，与当前快照状态分开呈现。',
      continuity: '项目身份、路径丢失状态和路径重新关联记录都会保留在同一个项目记录上。',
      initialization: '无需离开详情页，即可显式运行受支持的 `/gsd init` 流程。',
      timeline: '展示每个任务从开始到完成的区间，并附带实际耗时与当前预估。',
      sources: '后端快照适配器返回的各来源真实状态。',
    },
    actions: {
    ...
    empty: {
      ...
      init: '在你显式启动受支持的初始化引导流程之前，该项目会保持未初始化状态。',
    },
    messages: {
      ...
      noRelink: '尚未记录路径重新关联。',
      noPriorPath: '没有原路径记录。',
      pathLostTitle: '找不到登记的项目根目录。',
      pathLostCopy: (projectId) =>
        `面板会为 ${projectId} 保留最近一次良好快照、最新初始化任务和近期时间线，直到你把项目重新关联到新路径。`,
      relinkedTitle: '该项目已在不改变项目 ID 的情况下重新关联路径。',
      relinkedCopy: (projectId) => `当前快照、初始化历史和持久化时间线仍然归属于项目 ID ${projectId}。`,
      initStreamNote: (streamStatus) =>
        `初始化事件流当前为“${streamStatus}”。可重新加载详情查看持久化任务状态，等待事件流恢复。`,
      unavailable: '不可用',
      directoryEmpty: '该目录为空，因此项目仍处于未初始化状态。',
      directorySamples: '当前目录读取的示例条目：',
      truncated: '...已截断',
      ...
      noMilestones: '.gsd/gsd.db 中暂未发现里程碑记录。',
      noDependencies: '当前里程碑记录中还无法推断切片依赖。',
      unknown: '未知',
      yes: '是',
      no: '否',
      ...
      estimateUnavailable: '已完成的计时执行单元不足，暂无法估算。',
      noAgentUnits: '还没有记录最近的 Agent 执行单元。',
    ...
    errors: {
      ...
      relinkUnavailable: '只有当前项目路径被报告为丢失后，才能重新关联路径。',
      emptyRelinkPath: '重新关联之前请输入项目新的本地路径。',
    ...
    streamStatusMessages: {
      ...
      connected: '实时事件已连接。快照真实状态仍以项目 JSON 响应和监控元数据为准。',
